...
- **Keyboard shortcuts** (`?` opens help): Space play/pause, ←/→ prev/next track, Shift+←/→ seek, `/` search, `s` toggle mix sidebar, `m` neighbors tab, `n` neighbor mode on map, `a` add to draft, `d` draft tab, `f` toggle final, `p` play draft, `e`/`b` sort draft by energy/BPM, `z`/`k` toggle set zones/Camelot panels, `r` reset view, ↑/↓ mix queue when sidebar open, Esc peels filters/search/sidebar/neighbor mode/view
