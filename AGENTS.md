...
- **Keyboard shortcuts** (`?` opens help): Space play/pause, ←/→ prev/next track, Shift+←/→ seek, `/` search, `m` neighbors tab, `n` toggle neighbors on map, `a` add to draft, `d` draft tab, `f` toggle final, `p` play draft, `e`/`b` sort draft by energy/BPM, `r` reset view, ↑/↓ mix queue when neighbors/draft panel open, Esc peels filters/search/neighbors/view
